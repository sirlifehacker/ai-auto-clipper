@echo off
setlocal EnableDelayedExpansion
title AI Auto Clipper
color 0A

echo.
echo  ============================================
echo   AI Auto Clipper — Starting...
echo  ============================================
echo.

:: ── Guard: install must have been run first ────────────────────────────────
if not exist "python\python.exe" (
    echo  ERROR: App is not installed yet.
    echo.
    echo  Please double-click "install.bat" first,
    echo  then run this file again.
    echo.
    pause
    exit /b 1
)

:: ── Guard: .env must exist ─────────────────────────────────────────────────
if not exist ".env" (
    echo  No config file found — copying from template...
    copy ".env.example" ".env" >nul
)

:: ── Find a free port (default 8501, try up to 8510) ───────────────────────
set PORT=8501
:findport
netstat -an 2>nul | find ":%PORT% " >nul
if not errorlevel 1 (
    set /a PORT+=1
    if !PORT! GTR 8510 (
        echo  ERROR: No free port found between 8501-8510.
        pause
        exit /b 1
    )
    goto findport
)

:: ── Launch Streamlit ───────────────────────────────────────────────────────
echo  Starting app on http://localhost:!PORT!
echo  (Keep this window open while using the app)
echo  (Press any key here to stop the app)
echo.

start "" /B python\python.exe -m streamlit run app.py ^
    --server.port !PORT! ^
    --server.headless true ^
    --browser.gatherUsageStats false ^
    --theme.base light

:: ── Wait until Streamlit is ready, then open the browser ──────────────────
echo  Waiting for app to start...
set TRIES=0
:waitloop
timeout /t 2 /nobreak >nul
set /a TRIES+=1
python\python.exe -c "import urllib.request; urllib.request.urlopen('http://localhost:!PORT!')" >nul 2>&1
if not errorlevel 1 goto :ready
if !TRIES! LSS 20 goto :waitloop
echo  Taking longer than expected — opening browser anyway.

:ready
start "" "http://localhost:!PORT!"
echo  App is ready! Opening browser...
echo.
echo  ============================================
echo   AI Auto Clipper is running
echo   http://localhost:!PORT!
echo  ============================================
echo.
echo  Press any key to STOP the app.
pause >nul

:: ── Shut down cleanly on exit ──────────────────────────────────────────────
echo  Stopping...
for /f "tokens=5" %%p in ('netstat -ano 2^>nul ^| findstr ":!PORT! "') do (
    taskkill /f /pid %%p >nul 2>&1
)
echo  Done. Goodbye!
timeout /t 2 /nobreak >nul
endlocal
